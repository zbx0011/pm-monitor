...
def main():
    print("=" * 60)
    print("铂金历史价差分析 (广期所 PT vs CME)")
    print("=" * 60)
    print()
    
    # 1. 获取广期所铂金主力数据
    print("正在获取广期所铂金(PT)主力数据...")
    try:
        gfex_df = ak.futures_main_sina(symbol='PT0')
        print(f"  ✓ 获取到 {len(gfex_df)} 条广期所铂金数据")
    ...
    print(f"\n✓ 已生成 platinum_spread_analysis.json ({len(history)} 条记录)")
